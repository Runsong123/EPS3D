...
class LSegFeatureExtractor(LSegNet):
    ...
    # @torch.no_grad()
    def decode_logistics(self, image_features, labelset=''):
    ...
    @torch.no_grad()
    def decode_feature_text(self, device, labelset=''):
        # encode text
        if labelset == '':
            text = self.text
        else:
    ...
    def decode_feature_w_grad(self, image_features, labelset=''):
        imshape = image_features.shape
        ...
        image_features = image_features.permute(0,2,3,1).reshape(-1, self.out_c).float()
