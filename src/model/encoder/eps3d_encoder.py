...
from src.model.encoder.data_shims.bounds_shim import apply_bounds_shim
from src.model.encoder.data_shims.normalize_shim import apply_normalize_shim
from src.model.encoder.data_shims.patch_shim import apply_patch_shim
from src.model.encoder.data_shims.types import BatchedExample, DataShim
...
from .common.gaussian_adapter import (
    ...
    UnifiedSemanticGaussianAdapter,
    UnifiedPanopticGaussianAdapter,
)
from .common.sem2ins_fusion import Sem2InsFusion
...
class EncoderEPS3DCfg:
    ...
    voxelize: bool = False
    semantic_fea: bool = False
    gs_semantic_fea: int = 16
    instance_fea: bool = False
    gs_instance_fea: int = 32
    use_sem2ins: bool = False
...
class EncoderEPS3D(Encoder[EncoderEPS3DCfg]):
    backbone: nn.Module
    gaussian_adapter: GaussianAdapter

    def __init__(self, cfg: EncoderEPS3DCfg) -> None:
        super().__init__(cfg)
        ...
        self.semantic_fea = cfg.semantic_fea
        self.gs_semantic_fea = cfg.gs_semantic_fea
        self.instance_fea = cfg.instance_fea
        self.gs_instance_fea = cfg.gs_instance_fea

        if self.pose_free:
            if self.instance_fea and self.semantic_fea:
                self.gaussian_adapter = UnifiedPanopticGaussianAdapter(cfg.gaussian_adapter)
            elif self.semantic_fea:
                self.gaussian_adapter = UnifiedSemanticGaussianAdapter(cfg.gaussian_adapter)
            else:
        ...
        self.gaussian_feature_param_head = VGGT_DPT_GS_Head(
            dim_in=2048,
            patch_size=head_params.patch_size,
            output_dim=self.gs_semantic_fea,
            activation="norm_exp",
            conf_activation="expp1",
            features=head_params.feature_dim,
        )

        if self.instance_fea:
            self.gaussian_instance_param_head = VGGT_DPT_GS_Head(
                dim_in=2048,
                patch_size=head_params.patch_size,
                output_dim=self.gs_instance_fea,
                activation="norm_exp",
                conf_activation="expp1",
                features=head_params.feature_dim,
            )

        if cfg.use_sem2ins and self.instance_fea and self.semantic_fea:
            self.sem2ins_fusion = Sem2InsFusion(
                instance_dim=self.gs_instance_fea,
                semantic_dim=self.gs_semantic_fea,
                hidden_dim=32,
                w_init=0.1,
            )
    ...
    def voxelizaton_with_fusion(self, img_feat, pts3d, voxel_size, conf=None, mask = None):
        # img_feat: B*V, C, H, W
        # pts3d: B*V, 3, H, W
        V, C, H, W = img_feat.shape
        if mask is not None:
            pts3d_flatten = pts3d.permute(0, 2, 3, 1)[mask].reshape(-1,3)
            ...
            conf_flat = conf[mask].flatten()  # [B*V*N]
            anchor_feats_flat = img_feat.permute(0, 2, 3, 1)[mask]  # [B*V*N, ...]
        else:
    ...
    def forward(
        ...
        if self.semantic_fea:
            out_fea = self.gaussian_feature_param_head(
                aggregated_tokens_list,
                pts_all.flatten(0, 1).permute(0, 3, 1, 2),
                image,
                patch_start_idx=patch_start_idx,
        ...
        if self.instance_fea:
            out_instance_fea = self.gaussian_instance_param_head(
                aggregated_tokens_list,
                pts_all.flatten(0, 1).permute(0, 3, 1, 2),
                image,
                patch_start_idx=patch_start_idx,
        ...
        if self.cfg.use_sem2ins and self.instance_fea and self.semantic_fea:
            out_instance_fea = self.sem2ins_fusion(out_instance_fea, out_fea)
        ...
        if self.semantic_fea:
            anchor_semantic_feats = out_fea
            anchor_feats = torch.cat([anchor_feats, anchor_semantic_feats], dim=2)

        if self.instance_fea:
            anchor_instance_feats = out_instance_fea
            anchor_feats = torch.cat([anchor_feats, anchor_instance_feats], dim=2)
            ...
            if mask is not None:
                for b_i in range(b):
                    neural_pts, neural_feats = self.voxelizaton_with_fusion(
                        ...
                        self.voxel_size,
                        conf=conf[b_i],
                        mask=mask[b_i] ## torch.Size([1, 8, 448, 448])
                    )
        ...
        if self.instance_fea and self.semantic_fea:
            gaussians = self.gaussian_adapter.forward(
                ...
                d_semantic=self.gs_semantic_fea,
                d_instance=self.gs_instance_fea,
            )
        else:
            gaussians = self.gaussian_adapter.forward(
                neural_pts,
        ...
        if self.instance_fea and self.semantic_fea:
            print(
                ...
                f" semantic feature: mean {gaussians.semantics.mean()} \n"
                f" instance feature: mean {gaussians.instances.mean()}"
            )
        elif self.semantic_fea:
            print(
                f"Gaussians attributes: \n"
                f"opacities: mean: {gaussians.opacities.mean()}, min: {gaussians.opacities.min()}, max: {gaussians.opacities.max()} \n"
                f"scales: mean: {gaussians.scales.mean()}, min: {gaussians.scales.min()}, max: {gaussians.scales.max()} \n"
                f" feature: mean {gaussians.semantics.mean()}"
            )
        else:
            print(
                f"Gaussians attributes: \n"
