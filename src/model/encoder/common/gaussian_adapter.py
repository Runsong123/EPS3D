...
class UnifiedSemanticGaussianAdapter(GaussianAdapter):
    def forward(
        self,
        means: Float[Tensor, "*#batch 3"],
        # levels: Float[Tensor, "*#batch"],
        ...
        d_smeantic = raw_gaussians.shape[-1] - (3 + 4 +  3 * self.d_sh)
        scales, rotations, sh, semantics = raw_gaussians.split((3, 4, 3 * self.d_sh, d_smeantic), dim=-1)
...
class UnifiedPanopticGaussianAdapter(GaussianAdapter):
    def forward(
        self,
        means: Float[Tensor, "*#batch 3"],
        depths: Float[Tensor, "*#batch"],
        ...
        d_base = 3 + 4 + 3 * self.d_sh
        d_sem = d_semantic
        d_ins = raw_gaussians.shape[-1] - d_base - d_sem
        scales, rotations, sh, semantics, instances = raw_gaussians.split(
            (3, 4, 3 * self.d_sh, d_sem, d_ins), dim=-1
        )
