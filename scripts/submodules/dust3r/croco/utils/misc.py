...
def get_parameter_groups(model, weight_decay, layer_decay=1.0, skip_list=(), no_lr_scale_list=[]):
    ...
    for name, param in model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith(".bias") or name in skip_list or 'vggt' in name:
            if 'vggt' in name:
                if 'intrinsic_encoder' in name:
                    group_name = 'no_decay_scale_1'
                    this_weight_decay = 0.
                    scale = 1
                else:
                    group_name = 'no_decay_scale_0_1'
                    this_weight_decay = 0.
                    scale = 0.1
            else:
                group_name = "no_decay"
